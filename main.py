...
parser.add_argument(
    "--diff", "-d", type=float, default=0.05, help="Minimalna różnica w %% punktów"
)
...
def prepare_video_out(movie_path, fourcc):
    output_movie_path = f"{movie_path}_{time.strftime('%Y_%m_%d_%H_%M')}{MOVIE_PATH_EXTENSION}"
...
if __name__ == "__main__":
    ...
    ret, frame = cap.read()
    frame = cv2.resize(frame, (frame.shape[0] // args.scale, frame.shape[1] // args.scale))
    ...
    while True:
        ...
        if frame is not None:
            ...
            _, frame_bw = cv2.threshold(frame_bw, args.threshold, 255, cv2.THRESH_BINARY)
            ...
            cv2.putText(frame, time_part, (10, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 255))
            ...
            print(
                f"diff: {delta_p:.4f} | mean fps: {1/np.mean(fps_list):.2f}", end="\r"
            )
